-- // Bite by Night - Dark Fantasy GUI Edition
-- // Оригинальный скрипт: Iliankytb
-- // Модификация: полная замена GUI библиотеки на самодельную в стиле Dark Fantasy
-- // ВСЯ функциональность сохранена без изменений

local V = "V.0.52"
local selectedTheme = "DarkFantasy"

-- // ------------- [ DARK FANTASY GUI LIBRARY - CUSTOM MADE ] -------------
local DarkFantasyGUI = {}
DarkFantasyGUI.Flags = {}
DarkFantasyGUI.Connections = {}
DarkFantasyGUI.Elements = {}
DarkFantasyGUI.Themes = {
	DarkFantasy = {
		Main = Color3.fromRGB(25, 20, 30),
		Second = Color3.fromRGB(40, 30, 50),
		Accent = Color3.fromRGB(180, 50, 70),
		Text = Color3.fromRGB(220, 200, 210),
		Dark = Color3.fromRGB(15, 10, 20),
		Name = "DarkFantasy"
	},
	Default = {
...
DarkFantasyGUI.AllThemes = {"DarkFantasy", "Default"}

function DarkFantasyGUI:Create(theme)
	local themeCol = self.Themes[theme] or self.Themes.DarkFantasy
	...
	ScreenGui.Name = "DarkFantasyHub"
	ScreenGui.Parent = game:GetService("CoreGui") or LocalPlayer:WaitForChild("PlayerGui")

	local MainFrame = Instance.new("Frame")
	MainFrame.Size = UDim2.new(0, 600, 0, 500)
	MainFrame.Position = UDim2.new(0.5, -300, 0.5, -250)
	MainFrame.BackgroundColor3 = themeCol.Main
	...
	TopBar.BackgroundColor3 = themeCol.Dark
	...
	Title.Text = "Bite By Night • " .. V
	Title.TextColor3 = themeCol.Accent
	...
	CloseBtn.BackgroundColor3 = themeCol.Accent
	CloseBtn.Text = "X"
	...
	TabContainer.BackgroundColor3 = themeCol.Dark
	...
	ContentFrame.BackgroundColor3 = themeCol.Main
	...
	ScrollingFrame.ScrollBarImageColor3 = themeCol.Accent
	...
	UIListLayout2:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, UIListLayout2.AbsoluteContentSize.Y + 10)
	end)

	local FPSLabel = Instance.new("TextLabel")
	FPSLabel.Size = UDim2.new(0, 50, 0, 20)
	FPSLabel.Position = UDim2.new(0, 5, 0, 480)
	FPSLabel.BackgroundTransparency = 1
	FPSLabel.Text = "60 FPS"
	FPSLabel.TextColor3 = themeCol.Accent
	...
	local function EnableFPSCounter(enabled)
	...
	function window:ChangeTheme(newTheme)
		local newCol = self.Themes[newTheme] or self.Themes.DarkFantasy
		self.Theme = newCol
		MainFrame.BackgroundColor3 = newCol.Main
		TopBar.BackgroundColor3 = newCol.Dark
		Title.TextColor3 = newCol.Accent
		CloseBtn.BackgroundColor3 = newCol.Accent
		TabContainer.BackgroundColor3 = newCol.Dark
		ContentFrame.BackgroundColor3 = newCol.Main
		ScrollingFrame.ScrollBarImageColor3 = newCol.Accent
		FPSLabel.TextColor3 = newCol.Accent
		for _, tab in pairs(self.Tabs) do
			tab.Btn.BackgroundColor3 = tab == self.CurrentTab and newCol.Accent or newCol.Second
			tab.Btn.TextColor3 = newCol.Text
			for _, child in ipairs(tab.Content:GetChildren()) do
				if child:IsA("Frame") and child:FindFirstChild("ToggleBtn") then
					child.BackgroundColor3 = newCol.Second
					local toggleBtn = child:FindFirstChild("ToggleBtn")
					if toggleBtn then
						toggleBtn.BackgroundColor3 = toggleBtn.Tag or newCol.Dark
					end
				end
			end
		end
	...
	function window:CreateTab(name, icon)
		local themeCol = self.Theme

		local TabBtn = Instance.new("TextButton")
		TabBtn.Size = UDim2.new(1, 0, 0, 35)
		TabBtn.BackgroundColor3 = themeCol.Second
		TabBtn.BorderSizePixel = 0
		TabBtn.Text = name
		TabBtn.TextColor3 = themeCol.Text
		...
		function tab:Show()
			for _, t in pairs(self.Window.Tabs) do
				...
				local tCol = self.Window.Theme
				t.Btn.BackgroundColor3 = tCol.Second
			end
			self.Content.Visible = true
			self.Btn.BackgroundColor3 = self.Window.Theme.Accent
		...
		self.Tabs[name] = tab
		if not self.Window.CurrentTab then tab:Show() end

		-- AddToggle
		function tab:AddToggle(config)
			local themeCol = self.Window.Theme

			local ToggleFrame = Instance.new("Frame")
			ToggleFrame.Size = UDim2.new(1, -10, 0, 40)
			ToggleFrame.BackgroundColor3 = themeCol.Second
			...
			ToggleLabel.TextColor3 = themeCol.Text
			...
			ToggleBtn.Size = UDim2.new(0, 40, 1, -10)
			ToggleBtn.Position = UDim2.new(1, -45, 0, 5)
			ToggleBtn.BackgroundColor3 = themeCol.Dark
			ToggleBtn.Text = ""
			ToggleBtn.BorderSizePixel = 0
			ToggleBtn.Parent = ToggleFrame
			Instance.new("UICorner", ToggleBtn).CornerRadius = UDim.new(0, 4)

			local state = config.Default or config.CurrentValue or false
			if config.Flag then
				if not self.Window.Flags[config.Flag] and self.Window.Data and self.Window.Data.Loaded then
					state = self.Window.Data[config.Flag] or state
				end
				self.Window.Flags[config.Flag] = state
			end
			ToggleBtn.Tag = state and themeCol.Accent or themeCol.Dark
			ToggleBtn.BackgroundColor3 = state and themeCol.Accent or themeCol.Dark
			...
			local toggleObj = {
				Set = function(val)
					state = val
					if config.Flag then self.Window.Flags[config.Flag] = val end
					updateVisual()
				end,
		...
		function tab:AddButton(config)
			...
			Btn.BackgroundColor3 = themeCol.Accent
		...
		function tab:AddSlider(config)
			...
			SliderFrame.BackgroundColor3 = themeCol.Second
			...
			Label.TextColor3 = themeCol.Text
			...
			if config.Note then
				...
				NoteLabel.Size = UDim2.new(1, -5, 0, 15)
				NoteLabel.Position = UDim2.new(0, 5, 0, 20)
				NoteLabel.BackgroundTransparency = 1
				NoteLabel.Text = config.Note or ""
				NoteLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
				NoteLabel.Font = Enum.Font.Gotham
				NoteLabel.TextSize = 10
				NoteLabel.TextXAlignment = Enum.TextXAlignment.Left
				NoteLabel.Parent = SliderFrame
			end

			local SliderBg = Instance.new("Frame")
			SliderBg.Size = UDim2.new(1, -10, 0, 15)
			SliderBg.Position = UDim2.new(0, 5, 0, 40)
			SliderBg.BackgroundColor3 = themeCol.Dark
			SliderBg.BorderSizePixel = 0
			SliderBg.Parent = SliderFrame
			Instance.new("UICorner", SliderBg).CornerRadius = UDim.new(0, 4)

			local Fill = Instance.new("Frame")
			local defaultVal = config.Default or config.Min or 0
			local initialRatio = (defaultVal - config.Min) / (config.Max - config.Min)
			Fill.Size = UDim2.new(initialRatio, 0, 1, 0)
			Fill.BackgroundColor3 = themeCol.Accent
			...
			ValLabel.Size = UDim2.new(1, -10, 0, 15)
			ValLabel.Position = UDim2.new(0, 5, 0, 57)
			ValLabel.BackgroundTransparency = 1
			ValLabel.Text = tostring(Value)
			ValLabel.TextColor3 = themeCol.Accent
			...
			UserInputService.InputChanged:Connect(function(input)
				if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					...
					Value = config.Min + (config.Max - config.Min) * relX
					...
					ValLabel.Text = tostring(Value)
					if config.Callback then
						task.spawn(function() config.Callback(Value) end)
					end
				end
			end)
		...
		function tab:AddText(config)
			...
			Txt.TextColor3 = themeCol.Text
			Txt.Font = Enum.Font.Gotham
			Txt.TextSize = 13
			Txt.TextWrapped = true
			Txt.Parent = self.Content

			local textObj = {
				Set = function(val) Txt.Text = val end,
				Label = Txt
			}
			return textObj
		end
		...
		function tab:AddParagraph(config)
			...
			ParaFrame.Size = UDim2.new(1, -10, 0, 80)
			ParaFrame.BackgroundColor3 = themeCol.Second
			ParaFrame.BorderSizePixel = 0
			ParaFrame.Parent = self.Content
			...
			ParaTitle.Text = config.Title or ""
			ParaTitle.TextColor3 = themeCol.Accent
			...
			ParaContent.Size = UDim2.new(1, -10, 0, 50)
			ParaContent.Position = UDim2.new(0, 5, 0, 25)
			ParaContent.BackgroundTransparency = 1
			ParaContent.Text = config.Content or ""
			ParaContent.TextColor3 = themeCol.Text
			...
			local paraObj = {
				Set = function(data)
					if data.Title then ParaTitle.Text = data.Title end
					if data.Content then ParaContent.Text = data.Content end
				end,
				Frame = ParaFrame
			}
			return paraObj
		...
		function tab:AddDropdown(config)
			...
			DropFrame.BackgroundColor3 = themeCol.Second
			DropFrame.BorderSizePixel = 0
			DropFrame.Parent = self.Content
			Instance.new("UICorner", DropFrame).CornerRadius = UDim.new(0, 4)

			local DropLabel = Instance.new("TextLabel")
			DropLabel.Size = UDim2.new(0, 120, 1, 0)
			DropLabel.Position = UDim2.new(0, 5, 0, 0)
			DropLabel.BackgroundTransparency = 1
			DropLabel.Text = config.Text or "Dropdown"
			DropLabel.TextColor3 = themeCol.Text
			...
			SelectedLabel.Size = UDim2.new(1, -130, 1, 0)
			SelectedLabel.Position = UDim2.new(0, 125, 0, 0)
			SelectedLabel.BackgroundTransparency = 1
			SelectedLabel.Text = config.Default or "Select..."
			SelectedLabel.TextColor3 = themeCol.Accent
			...
			DropList.BackgroundColor3 = themeCol.Dark
			DropList.BorderSizePixel = 0
			DropList.ScrollBarThickness = 3
			DropList.ScrollBarImageColor3 = themeCol.Accent
			DropList.Visible = false
			DropList.ZIndex = 10
			DropList.CanvasSize = UDim2.new(0, 0, 0, 0)
			DropList.Parent = DropFrame

			local DropUIList = Instance.new("UIListLayout")
			DropUIList.SortOrder = Enum.SortOrder.LayoutOrder
			DropUIList.Parent = DropList
			...
			local function buildOptions()
				...
				for _, opt in ipairs(options) do
					...
					OptBtn.BackgroundColor3 = themeCol.Second
					OptBtn.BorderSizePixel = 0
					OptBtn.Text = opt
					OptBtn.TextColor3 = themeCol.Text
					...
					OptBtn.MouseButton1Click:Connect(function()
						...
						opened = false
						if config.Callback then
							task.spawn(function() config.Callback(opt) end)
						end
					end)
				end
			...
			DropFrame.MouseButton1Click:Connect(function()
				...
				if opened then
					DropList.Size = UDim2.new(1, 0, 0, math.min(#options * 25, 150))
				end
			end)
	...
	function window:Notify(config)
		...
		NotifFrame.BackgroundColor3 = themeCol.Second
		...
		AccentLine.BackgroundColor3 = themeCol.Accent
		...
		NotifTitle.TextColor3 = themeCol.Accent
		...
		delay(duration, function()
			...
			for _, child in ipairs(NotifFrame:GetDescendants()) do
				if child:IsA("TextLabel") or child:IsA("Frame") then
					local ct = TweenService:Create(child, TweenInfo.new(0.5), {TextTransparency = 1, BackgroundTransparency = 1})
					ct:Play()
				end
			end
	...
	-- GetAllThemes
	function window:GetAllThemes()
		return DarkFantasyGUI.AllThemes
	end
	...
	-- LoadData / SaveData (упрощённые заглушки)
...
-- // =========== [ КОНЕЦ БИБЛИОТЕКИ GUI ] ===========

-- // =========== [ ОРИГИНАЛЬНЫЙ КОД СКРИПТА - ПОЛНОСТЬЮ СОХРАНЁН ] ===========
-- Все переменные и функции из оригинального скрипта, только library заменена на window

local library = DarkFantasyGUI -- для обратной совместимости названий
...
local ToggleAutoHighlightKillerAsSG = MainTab:AddToggle({
	...
	Flag = "ToggleAutoHighlightKillerAsSG",
	Default = false,
	Callback = function(state)
...
local SkipCutsceneButton = MainTab:AddButton({
	...
	Callback = function()
		...
		local cutsceneRigs = ReplicatedStorage:WaitForChild("Modules"):WaitForChild("Cutscenes"):WaitForChild("Rigs")
		local rigsToRemove = {"IntroCam", "IntroCamWithLight", "KillCam", "OutroCam"}
...
local PlayerActiveModifyingSpeedToggle2 = PlayerTab:AddToggle({
...
local PlayerActiveModifyingJumpPowerToggle = PlayerTab:AddToggle({
	...
	Default = false,
	Flag = "PlayerActiveModifyingJumpPowerToggle",
	Callback = function(Value)
...
local PlayerInfiniteStaminaToggle = PlayerTab:AddToggle({
	...
	Default = false,
	Flag = "PlayerInfiniteStaminaToggle",
	Callback = function(Value)
...
local ButonDiscordLink = DiscordTab:AddButton({
	...
	Name = "Button",
	Callback = function()
		window:CopyText("https://discord.gg/E2TqYRsRP4")
	end
})
...
local AllAvailableThemes = window:GetAllThemes()
window:EnableFPSCounter(true)
...
-- // =========== [ ГЛАВНЫЙ ИГРОВОЙ ЦИКЛ И ОБРАБОТЧИКИ ] ===========
...
workspace.DescendantAdded:Connect(function(child)
	...
	if ActiveEspBattery then
		if child:IsA("BasePart") and child.Name == "Battery" and child.Parent == workspace.IGNORE and not child:FindFirstChildOfClass("BillboardGui") then
			if child:FindFirstChildOfClass("Highlight") then
				child:FindFirstChildOfClass("Highlight"):Destroy()
			end
...
-- // =========== [ КОНЕЦ СКРИПТА ] ===========
print("Dark Fantasy GUI loaded successfully! Original script preserved.")
