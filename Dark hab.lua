...
local V = "V.0.52"
local selectedTheme = "Dark Fantasy" -- Изменено на тему Dark Fantasy

library:CreateWindow({
    Title = "Dark fantasy hab", -- Title of the script
    ...
    local PlayerRunSpeedSlider = PlayerTab:AddSlider({
        Text = "Run Speed",
        Name = "PlayerRunSpeedSlider",
        Min = 0,
        Max = 50,
        Default = 24,
        Flag = "Slider1",
    ...
    local PlayerActiveRunSpeedToggle = PlayerTab:AddToggle({
        Text = "Active Modifying Run Speed",
        Name = "PlayerActiveRunSpeedToggle",
        Default = false,
        Flag = "ButtonSpeed",
        Callback = function(Value)
    ...
    local PlayerWalkSpeedSlider = PlayerTab:AddSlider({
        Text = "Walk Speed",
        Name = "PlayerWalkSpeedSlider",
        Min = 0,
        Max = 50,
        Default = 15,
        Flag = "Slider2",
    ...
    local PlayerActiveWalkSpeedToggle = PlayerTab:AddToggle({
        Text = "Active Modifying Walk Speed",
        Name = "PlayerActiveWalkSpeedToggle",
        Default = false,
        Flag = "ButtonSpeed2",
        Callback = function(Value)
    ...
    local PlayerActiveJumpPowerToggle = PlayerTab:AddToggle({
        Text = "Active Modifying Jump Power",
        Name = "PlayerActiveJumpPowerToggle",
        Default = false,
        Flag = "PlayerActiveJumpPowerToggle",
        Callback = function(Value)
    ...
    local AutoEscapeButton = MainTab:AddToggle({
        Text = "Auto Escape",
        Name = "AutoEscapeButton",
        Default = false,
        Flag = "AutoEscapeButton",
        Callback = function(Value)
            AutoEscape = Value
        end,
    })
    ...
    workspace.DescendantAdded:Connect(function(child)
        ...
        if child:IsA("Highlight") and child.Name == "Highlight" and child.Parent == workspace.PLAYERS.KILLER:FindFirstChildOfClass("Model") then
            local Character = game.Players.LocalPlayer.Character
            if FighterAutoParry and HrpPlayer and CanParry and Character and not Character:GetAttribute("IFrames") and not Character:GetAttribute("InAbility") and not Character:GetAttribute("Stun") and Character:GetAttribute("Team") == "Survivor" and Character:GetAttribute("Character") == "Survivor-Fighter" then
        ...
        if ActiveEspSurvivors then
            ...
            if child.Parent == workspace.PLAYERS.ALIVE and child:IsA("Model") and child.PrimaryPart and not child.PrimaryPart:FindFirstChildOfClass("BillboardGui") then
                if GetPByChar then
                    ...
                    CreateEsp(child, Color3.fromRGB(0, 255, 0), child.Name .. " " .. child:GetAttribute("Character"), child.PrimaryPart)
                end
            end
        end
        if ActiveEspKillers then
            ...
            if child.Parent and child.Parent == workspace.PLAYERS.KILLER and child:IsA("Model") and child:FindFirstChild("RootPart") and not child.RootPart:FindFirstChildOfClass("BillboardGui") then
                if GetPByChar then
                    ...
                    if GetPByChar.Character and GetPByChar.Character:GetAttribute("Character") == "Ennard" then
                    ...
                    CreateEsp(child, Color3.fromRGB(255, 0, 0), child.Name .. " " .. child:GetAttribute("Character"), PART)
                end
            end
        end
        if ActiveEspGen then
    ...
    workspace.DescendantRemoving:Connect(function(child)
        if child:IsA("Model") then
            ...
            if ActiveEspKillers then
                if child:IsDescendantOf(workspace.PLAYERS.KILLER) then
                    if (child:FindFirstChild("RootPart") or child:FindFirstChild("HumanoidRootPart")) then
                        ...
                        if child:FindFirstChildOfClass("Highlight") and PART and PART:FindFirstChildOfClass("BillboardGui") then
                            KeepEsp(child, PART)
                        end
                    end
                end
            ...
            if ActiveEspGen and workspace.MAPS:FindFirstChild("GAME MAP") then
                ...
                if map.Generators and child:IsDescendantOf(map.Generators) and child.PrimaryPart then
                    if child:FindFirstChildOfClass("Highlight") and child.PrimaryPart:FindFirstChildOfClass("BillboardGui") then
                        KeepEsp(child, child.PrimaryPart)
                    end
                end
            end
            if ActiveEspFuseBoxes and workspace.MAPS:FindFirstChild("GAME MAP") and workspace.MAPS["GAME MAP"]:FindFirstChild("FuseBoxes") then
                local map = workspace.MAPS["GAME MAP"]
                if map.FuseBoxes and child:IsDescendantOf(map.FuseBoxes) and child.PrimaryPart then
                    if child:FindFirstChildOfClass("Highlight") and child.PrimaryPart:FindFirstChildOfClass("BillboardGui") then
                        KeepEsp(child, child.PrimaryPart)
                    end
                end
